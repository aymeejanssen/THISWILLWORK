...
const OnboardingFlow = ({
    ...
    return <Card className="w-full max-w-2xl mx-auto">
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            }).map((_, i) => <div key={i} className={`w-2 h-2 rounded-full ${i < responses.length ? 'bg-purple-500' : 'bg-gray-200'}`} />)}
            </div>
          </div>
  ...
  if (currentStep === 0) {
    return <div className="p-8 max-h-[80vh] overflow-y-auto">
        ...
        <div className="grid gap-4 max-w-4xl mx-auto">
          {concerns.map(concern => <Card key={concern.id} className="cursor-pointer hover:shadow-md transition-all duration-200 group" onClick={() => handleConcernSelect(concern.id)}>
  ...
  return <div className="p-8 max-h-[80vh] overflow-y-auto">
      <div className="mb-6">
        <Button variant="ghost" onClick={() => currentStep > 1 ? setCurrentStep(currentStep - 1) : setCurrentStep(0)} className="mb-4">
          ← Back
        </Button>
      </div>
      {renderQuestions()}
...
const QuestionInput = ({
  ...
  return <form onSubmit={handleSubmit} className="space-y-4">
      <Textarea value={response} onChange={e => setResponse(e.target.value)} placeholder="Type your answer here, or use the microphone to speak..." className="min-h-[120px] resize-none" autoFocus />
      
      <div className="flex justify-between items-center">
        <VoiceInput onTranscript={handleVoiceTranscript} isListening={isListening} onListeningChange={setIsListening} />
        
        <Button type="submit" disabled={!response.trim()} className="bg-purple-600 hover:bg-purple-700">
          Continue
          <ArrowRight className="ml-2 h-4 w-4" />
        </Button>
      </div>
      
      {isListening && <div className="text-sm text-gray-600 italic">
